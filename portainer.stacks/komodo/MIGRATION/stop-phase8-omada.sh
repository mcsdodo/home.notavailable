#!/bin/bash
# Stop and remove omada containers before phase 8
# Stacks: omada
#
# Usage:
#   ./stop-phase8-omada.sh           # Stop and remove
#   ./stop-phase8-omada.sh --status  # Just show status
...
show_status() {
    echo -e "${CYAN}Phase 8 containers status on $HOST_NAME ($HOST):${NC}"
    echo ""
...
stop_and_remove() {
    echo -e "${CYAN}Stopping and removing phase 8 containers on $HOST_NAME ($HOST)...${NC}"
    echo ""
